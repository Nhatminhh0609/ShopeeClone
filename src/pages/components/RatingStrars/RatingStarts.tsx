...
export default function RatingStars({ queryConfig }: Props) {
  ...
  const handleFillterStart = (ratingFillter: number) => {
    navigate({
      pathname: path.home,
      search: createSearchParams({
        ...queryConfig,
        rating_filter: String(ratingFillter)
      }).toString()
    })
  }
  return (
      ...
      {Array(5)
        ...
        .map((_, index) => (
          <li className='py-1 pl-2' key={index}>
            <div className='flex items-center text-sm cursor-pointer' onClick={() => handleFillterStart(5 - index)}>
              {Array(5)
                .fill(0)
                .map((_, indexStart) => {
